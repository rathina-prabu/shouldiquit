...
export function ShareButtons({ shareUrl, tier, score, weakestModule }: Props) {
  ...
  // X (Twitter) accepts text + url separately so the URL gets a clean preview card.
  const xUrl = `https://twitter.com/intent/tweet?text=${encodeURIComponent(`Just got diagnosed: ${TIER_TEXT[tier] ?? tier} (${score}/100). The ${moduleName} problem. Take yours:`)}&url=${encodeURIComponent(shareUrl)}`
  // LinkedIn share API only accepts a URL — preview is pulled from OG meta.
  ...
  const btnCls =
    "flex-1 flex items-center justify-center py-3 border border-ink hover:bg-ink hover:text-paper transition-colors"
    ...
    <div className="mt-1 mb-6 flex flex-col gap-3">
      ...
      <div className="flex gap-2">
        ...
        <button
          ...
          className={btnCls}
        >
          {copied === "native" ? <span className="text-[12px]">Copied!</span> : <IconShare />}
        </button>
      </div>
      <button
        onClick={copyLink}
